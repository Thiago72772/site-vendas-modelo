...
export default function AdminDashboard() {
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center py-20">
          <Clock className="w-5 h-5 text-dim animate-pulse" />
        </div>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout>
      <h1 className="text-lg font-semibold text-strong mb-0.5">Dashboard</h1>
      <p className="text-sm text-mid mb-5">Visão geral do seu negócio hoje</p>

      {/* Stat cards — border only, no shadow */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <div className="card p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-2xs text-dim">Hoje</span>
          </div>
          <p className="text-xl font-semibold text-strong">{formatCurrency(totalVendido)}</p>
          <p className="text-xs text-mid mt-0.5">Total vendido</p>
        </div>
        ...
        <div className="card p-4">
          ...
          <p className="text-xs text-mid mt-0.5">Pedidos hoje</p>
        </div>
      ...
      <div className="card p-4 mb-6">
        <h2 className="text-xs font-semibold text-mid uppercase tracking-wide mb-3">Produtos mais vendidos (7 dias)</h2>
        {topProdutos.length === 0 ? (
          <p className="text-sm text-dim py-4 text-center">Sem dados suficientes ainda.</p>
        ) : (
          <div className="space-y-2">
            {topProdutos.map((produto, i) => (
              <div key={i}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm text-strong truncate">{produto.nome}</span>
                  <span className="text-sm font-medium text-strong">{produto.total}x</span>
                </div>
      ...
      {/* Ongoing orders */}
      <div className="card p-4">
        <h2 className="text-xs font-semibold text-mid uppercase tracking-wide mb-3">Pedidos em andamento</h2>
        {pedidosAndamento.length === 0 ? (
          <p className="text-sm text-dim py-4 text-center">Nenhum pedido em andamento.</p>
        ) : (
          <div className="space-y-1">
            {pedidosAndamento.map((pedido) => (
              <div
                key={pedido.id}
                className="flex items-center justify-between p-2.5 rounded-md hover:bg-raised transition-colors"
              >
                <div className="min-w-0">
                  <p className="text-sm font-medium text-strong truncate">
                    {pedido.clientes?.nome ?? 'Cliente'}
                  </p>
                  <p className="text-2xs text-dim">{formatDate(pedido.created_at)}</p>
                </div>
                <div className="flex items-center gap-2.5 shrink-0">
                  <span className="text-2xs font-medium px-2 py-0.5 rounded-sm bg-primary-600/15 text-primary-400">
